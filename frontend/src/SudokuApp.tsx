...
function SudokuApp() {
  const [puzzle, setPuzzle] = useState<SudokuPuzzleData | null>(null);
  ...
  useEffect(() => {
    const fetchPuzzle = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch('/api/sudoku/puzzle');
        if (!response.ok) {
  ...
  const handleSetUsername = (name: string) => {
    ...
    if (trimmed.length > 0 && trimmed.length <= 50) {
      setUsername(trimmed);
      localStorage.setItem('sudokuUsername', trimmed);
  ...
  const handlePuzzleComplete = async (completionTimeMs: number) => {
    ...
    try {
      await fetch('/api/sudoku/leaderboard/submit', {
        method: 'POST',
  ...
  return (
    <div className="app">
      ...
      <footer className="footer">
        <p>Solve the daily 3×3 Sudoku puzzle</p>
